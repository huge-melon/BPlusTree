...
count=5000 # 12小时测试，日志每10s记录一次，因此循环4500次
...
do
    let count--
    sys_mess=`pidstat -uIr -h -G "test" 10 1|sed -n '4p'|awk '{print $1,$3,$8,$13,$14}'` # 每隔十秒进行一次采样
    ...
    output_mess=${sys[0]}' 进程id:'${sys[1]}'  CPU占用率:'${sys[2]}'%  已占用内存:'${sys[3]}'KB  内存占用率:'${sys[4]}'%'
